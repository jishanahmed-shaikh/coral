...
| [cloudflare](#cloudflare) | `http` | Query zones, DNS records, accounts, members, Workers scripts, R2 buckets, and audit logs from Cloudflare. |
...
Use the "Read all resources" template or create a custom token with
Zone:Read, DNS:Read, Account:Read, and Workers Scripts:Read permissions.
See [Cloudflare API token docs](https://developers.cloudflare.com/fundamentals/api/get-started/create-token/).

`CLOUDFLARE_ACCOUNT_ID` (required)

Your Cloudflare account ID. Find it on the right-hand sidebar of any
zone overview page, or run:
